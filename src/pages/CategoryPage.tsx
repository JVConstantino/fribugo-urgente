...
export default function CategoryPage() {
  ...
  const fetchArticles = useCallback(
    async (categoryId: string, pageNum: number, append: boolean) => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        await fetchArticles(cat.id, 0, false);
      } catch {
        if (!cancelled) setNotFound(true);
      } finally {
  ...
  const handleLoadMore = async () => {
    ...
    setLoadingMore(true);
    await fetchArticles(category.id, nextPage, true);
    setPage(nextPage);
  ...
  if (notFound || !category) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center">
        <h1 className="mb-4 text-3xl font-bold">Categoria nao encontrada</h1>
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content={`Noticias de ${category.name} no ${APP_NAME}`} />
      </Helmet>
